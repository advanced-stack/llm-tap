...
PLACE_A_SRC = Place(
    name="PlaceA", description="Source A", type=SOURCE, token_type=TT_INT
)
PLACE_B_SINK = Place(
    name="PlaceB", description="Sink B", type=SINK, token_type=TT_BOOL
)
PLACE_C_SRC = Place(
    name="PlaceC", description="Source C", type=SOURCE, token_type=TT_FLOAT
)
PLACE_D_SINK_INT = Place(
    name="PlaceD", description="Sink D", type=SINK, token_type=TT_INT
)
...
def test_get_token_names_single():
    ...
    register_token_type(
        PLACE_A_SRC, TT_INT
    )  # register_token_type is called by register_place implicitly
...
def test_get_token_names_duplicates_through_places():
    ...
    place_a_dup_token = Place(
        name="PlaceADup",
        description="Source ADup",
        type=SOURCE,
        token_type=TT_INT,
    )
    register_place(PLACE_A_SRC)
...
def test_get_places_no_match_filters():
    ...
    filtered = get_places(
        is_sink, is_bool_token
    )  # No sink with bool token registered
