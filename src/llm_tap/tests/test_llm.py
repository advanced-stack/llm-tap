...
def test_to_json_schema_nested_data():
    ...
    assert schema["properties"]["item"]["title"] == "SimpleItem"
    ...
    assert "maybe_num" not in schema["required"]
    assert sorted(schema["required"]) == sorted(["item", "count"])
    teardown_function()
...
def test_to_json_schema_with_enum():
    ...
    assert sorted(schema["properties"]["my_enum"]["enum"]) == sorted(
        ["A", "B"]
    )
    assert "my_str_enum" in schema["properties"]
    assert schema["properties"]["my_str_enum"]["type"] == "string"
    assert sorted(schema["properties"]["my_str_enum"]["enum"]) == sorted(
        ["X", "Y"]
    )
    teardown_function()
...
def test_to_json_schema_union_type():
    ...
    choice_a_schema = next(
        s
        for s in choice_props["anyOf"]
        if s["properties"]["name"]["enum"] == ["ChoiceA"]
    )
    choice_b_schema = next(
        s
        for s in choice_props["anyOf"]
        if s["properties"]["name"]["enum"] == ["ChoiceB"]
    )
...
def test_from_dict_nested_data():
    ...
    to_json_schema(
        NestedData
    )  # Populates mappings for NestedData and SimpleItem
...
def test_from_dict_union_type_choice_a():
    ...
    data = {
        "choice_field": {"name": "ChoiceA", "arguments": {"value_a": "hello"}}
    }
